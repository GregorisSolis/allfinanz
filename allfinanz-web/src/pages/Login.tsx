...
export function Login(){
	...
	async function setLogin(event: FormEvent){
		...
		if(!password || !email){
			...
			.catch(err => {
				setMessage('Ups.. algo no esta bien, revisa tu email o contraseña.')
			})
		}
		...
		<div className="w-full min-h-96 text-white flex my-16">
			<div className="m-auto min-w-2/4 w-1/4 rounded bg-brand-200 shadow-lg p-4">
				<form onSubmit={setLogin} className="flex flex-col text-center">
					...
					<a className="hover:text-sky-500 hover:underline transition" href="/olvide-mi-contraseña">Olvide mi contraseña</a>
				</form>
			</div>
		</div>
	)
